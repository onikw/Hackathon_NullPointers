...
from sun import get_sunshine_data
from calculate_heures import calculate_hours_from_day_time, most_expensive_segment
...
def create_analysis_chart(energy_needed, max_power, solar_panel_power):
    ...
    ax1.step(hours, display_prices, color='#1f77b4', linewidth=2.5, label='Cena energii', where='post')
    ...
    ax2 = ax1.twinx()
    ax2.plot(hours, solar_production, color='#FDC200', linewidth=2.5, label='Produkcja energii z PV')
    ...
    avg_price_during_charging = sum([display_prices[int(h) % 24] for h in charging_x]) / len(charging_x) if charging_x else 0
    avg_price_overall = sum(display_prices) / len(display_prices)
    savings_percent = (1 - avg_price_during_charging/avg_price_overall) * 100 if avg_price_overall > 0 else 0
